...
#ifdef zlljets_Axe_noSkim_cxx

zlljets_Axe_noSkim::zlljets_Axe_noSkim(TTree *tree) : edimarcoTree_noSkim(tree) {
...
void zlljets_Axe_noSkim::loop(const char* configFileName)
{
   ...
   if (fabs(LEP_PDG_ID) == 13) {  // if we have Z -> mumu do stuff...
     ...
     strcpy(ROOT_FNAME,"zmumujets_Axe_noSkim.root");
     strcpy(TXT_FNAME,"zmumujets_Axe_noSkim.txt");
     strcpy(TEX_FNAME,"zmumujets_Axe_noSkim.tex");
     strcpy(FLAVOUR,"mu");
     ...
     strcpy(ROOT_FNAME,"zeejets_Axe_noSkim.root");
     strcpy(TXT_FNAME,"zeejets_Axe_noSkim.txt");
     strcpy(TEX_FNAME,"zeejets_Axe_noSkim.tex");
     strcpy(FLAVOUR,"ele");
   ...
   resoAndResponse.append(oppChargeLeptonsC.get2ToId() + twoLeptonsC.get2ToId());  // maybe it would be better to ask for 2 loose leptons
   ...
   Double_t ZptBinEdges[] = {50., 60., 70., 80., 90., 100., 110., 120., 130., 140., 150., 160., 170., 180., 190., 200., 210., 220., 230., 240., 250., 260., 270., 280., 290., 310., 330., 350., 370., 400., 430., 460., 490., 530., 570, 610., 650., 700., 800.};
   ...
   Double_t HTbin[] = {100,200,400,600,100000};
   TH1D *H_AccTimesEff_noW_HTbin[3];
   for (Int_t i = 0; i < 3; i++) {
     H_AccTimesEff_noW_HTbin[i] = new TH1D(Form("H_AccTimesEff_noW_HTbin%3.0lfto%3.0lf",HTbin[i],HTbin[i+1]),"",nMetBins,metBinEdges);
   }
   ...
   cout<<"zlljets_Axe_noSkim::loop()"<<endl;
   ...
   for (Int_t jentry=0; jentry<nentries; jentry++) {
	 ...
	 if ( metNoLepPt > METNOLEP_START ) HLT_passed_flag = 1; 	 
